...
(Old link: https://github.com/openhab/openhab-core/blob/master/bundles/org.openhab.ui.homebuilder/web/src/textItems.js)
*/
...
function generateLinks(links) {
	var rtn = new Object();
	let linksAdd = [
		...links.map(link => {return ["curl -X PUT --header \"Content-Type: application/json\" --header \"Accept: application/json\" -d \"{}\" \"http://localhost:8080/rest/links/" + link.itemName + "/" + encodeURIComponent(link.channelUID) + "\""]})
	];

	let linksDelete = [
		...links.map(link => {return ["curl -X DELETE --header \"Accept: application/json\" \"http://localhost:8080/rest/links/" + link.itemName + "/" + encodeURIComponent(link.channelUID) + "\""]})
	];
